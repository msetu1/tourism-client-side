...
const MyList = () => {
  ...
  useEffect(() => {
    fetch(`https://tourism-client-server.vercel.app/myList/${user?.email}`)
      .then((res) => res.json())
  ...
  const handleDeleted = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://tourism-client-server.vercel.app/delete/${id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
  ...
  return (
    <div>
      <Helmet>
        <title> My List | rf Travels</title>
      </Helmet>
      <CommonBanner
        headerText={"Your personalized travel bucket list"}
        subText={"My List"}
      />
