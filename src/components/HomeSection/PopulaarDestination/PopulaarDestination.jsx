...
const PopularDestination = () => {
  ...
  useEffect(() => {
    fetch("https://tourism-client-server.vercel.app/allCountries").then((res) =>
      res.json().then((data) => setCountry(data))
    );
  }, []);
  return (
    <div className="lg:max-w-7xl mx-auto px-5 md:px-10 lg:px-0 lg:mb-32 md:mb-16 mb-10">
      <h2 className="mb-8 text-2xl lg:text-4xl md:text-3xl font-bold">
        Our Popular <span className="text-[#22d3ee]">Destinations</span>{" "}
      </h2>
              ...
              <div>
                <div className=" bg-base-100 lg:w-96 h-[480px]   shadow-xl">
                  ...
                  <div className="px-3 pt-8 pb-6">
                    <h2 className="text-2xl font-bold mb-3">{country.title}</h2>
                    <p className="text-gray-500 mb-5">{country.description}</p>
                  </div>
                </div>
              </div>
            </Link>
